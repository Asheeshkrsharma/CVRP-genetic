...
/// 4. GIF animation support, arrow and standard command building using osstream
///    by Asheesh Sharma (28/11/16)
/// Requirements:
...
class Gnuplot
{
    ...
    public:
        ...
        /// plot vectors with two set of points
        Gnuplot(const int &x,
                const int &y,
                const int &z,
                const int &a,
    ...
    Gnuplot& savetogif(const std::string &filename = "gnuplot_output");
    ...
    Gnuplot& set_pointtype(const int pointtype = 1);
    /// turns grid on/off
    ...
    Gnuplot& plot_xy(const X& x, const Y& y, const std::string &title = "", const int &pt=1, const std::string &np = "");
...
//
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y, const std::string &title, const int &pt, const std::string &np)
{
    ...
    plotfile_xy(name, 1, 2, title, pt, np);
    return *this;
}
...
//------------------------------------------------------------------------------
//
// saves a gnuplot session to a gif file
//
Gnuplot& Gnuplot::savetogif(const std::string &filename)
{
    cmd("set term gif animate delay 5 enhanced");

    std::ostringstream cmdstr;
    cmdstr << "set output \"" << filename << ".gif\"";
    cmd(cmdstr.str());
...
// scales the size of the points used in plots
//
Gnuplot& Gnuplot::set_pointtype(const int pointtype)
{
    std::ostringstream cmdstr;
    cmdstr << "set pointtype " << pointtype;
    cmd(cmdstr.str());
